...
const createBooking = async (req, res) => {
    try {
        ...
        // Validate thời gian
        if (start < now) {
            return res.send('<script>alert("Lỗi: Thời gian nhận xe không được ở quá khứ!"); window.history.back();</script>');
        }
        if (end <= start) {
        ...
        if (overlapBooking) {
            return res.send('<script>alert("Xe đã bận (hoặc đang chờ duyệt) trong khung giờ này! Vui lòng chọn giờ khác."); window.history.back();</script>');
        }
        ...
        // --- [THAY ĐỔI QUAN TRỌNG] ---
        // Tạo đơn với trạng thái PENDING (Chờ duyệt)
        await Booking.create({
            ...
            endDate,
            totalPrice,
            status: 'PENDING' // <--- Không được để CONFIRMED
        });

        // Thông báo khách hàng chờ duyệt
        res.send('<script>alert("Gửi yêu cầu thành công! Vui lòng chờ Admin duyệt."); window.location.href="/bookings/my-bookings";</script>');
